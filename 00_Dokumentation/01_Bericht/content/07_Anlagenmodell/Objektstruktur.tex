\section{Definition der Objekt-Struktur} \label{Objektstruktur}
	...
	Die Steuerungs-, Betriebs- und Prozesselemente erfüllen somit die gleichen Aufgaben wie beim Skill. Zwischen der Struktur von Objekten und Skills gibt es bewusste Parallelen, welche die Verständlichkeit und Übersichtlichkeit der Gesamtsoftware verbessern. Die Steuerungselemente umfassen die Gleichen Methoden und Eigenschaften wie die Skills (\ref{tab:Objekt_Steuerungselemente}) (Umsetzung dieser unterscheidet sich jedoch). 
	...
	\begin{table}[ht]
		...
		\begin{bfhTabular}{| c | l | L{3.5cm} | L{5cm} |}
			Art: 		& Bezeichnung:		& Typ:		& Beschreibung:								
			\\\hline
			Methode		& \verb|M_Start|		& BOOL			& Methode zum Starten des Objektes
			\\\hline
			Methode		& \verb|M_Stop|			& BOOL			& Methode zum Stoppen des Objektes
			\\\hline
			Methode		& \verb|M_Reset|		& BOOL			& Methode zum Resetten des Objektes
			\\\hline
			Eigenschaft	& \verb|P_State|	& eObjectAktorState / eObjectSensorState & Eigenschaft des aktuellen Zustandes
		\end{bfhTabular}
	...
	Die Eigenschaft wird mit einem benutzerdefinierten Datentyp (\ref{tab:Datentypen_Benutzerdefiniert_Objekt}) umgesetzt, welche die Zustände des Objektes abbildet. Somit ist immer klar, in welchem Zustand sich das Objekt im Moment befindet. Dabei unterscheidet man zwischen Aktor und Sensor. 
